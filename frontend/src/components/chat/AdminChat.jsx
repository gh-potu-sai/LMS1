        ...
        <div className="adminchat-panel">
          ...
          >
            {selectedCustomer &&
              ...
              chats[selectedCustomer].map((msg) => {
                const isAdminMsg = msg.senderType === "ADMIN";
                const isMine =
                  isAdminMsg &&
                  parsedAdminId != null &&
                  String(msg.adminId) === String(parsedAdminId);

                // 🔒 Do NOT trust msg.adminName from server for labeling
                const senderLabel = isAdminMsg
                  ? isMine && adminDisplayName
                    ? `Admin (${adminDisplayName})`
                    : "Admin"
                  : customerNames[msg.customerId] || "Customer";

                const side = isAdminMsg ? "right" : "left";

                return (
                  <div
                    key={msg.id}
                    className={`adminchat-msg adminchat-side-${side}`}
                  >
